...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    int xStart = static_cast<int>(std::floor(xMin));
    int xEnd = static_cast<int>(std::ceil(xMax));
    int yStart = static_cast<int>(std::floor(yMin));
    int yEnd = static_cast<int>(std::ceil(yMax));
    ...
    for (int i = xStart; i <= xEnd; i++)
    {
        for (int j = yStart; j <= yEnd; j++)
        {
            ...
            if (insideTriangle(x, y, t.v))
            {
                ...
                // Explanation:
                // Z = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w()); is LINEAR (in View Space).
                // We can consider it as " 1 / w(x, y) ".
                // When doing linear(barycentric) interpolation，need to be divided by w because we need LINEARITY.
                // e.g. alpha * v[0].z() / v[0].w() + ... + ..., we can consider it as " z(x, y)/w(x, y) ".
                // So finally, need z(x, y)/w(x, y) divided by 1 / w(x, y) to recover the z(x, y),
                // that's what "*= Z" means.

                // 我靠，写英文还是有点折磨。这样总结吧：
                // 所谓重心坐标插值，无论是 color，normal 还是 depth，
                // 其实就是从 *屏幕空间* 的射影结构出发，
                // 通过考虑 1/w 的线性性以及重心坐标插值，最终插值出在 *world/view/model 空间* 是正常的三角形内部量。
                ...
                // Depth test
                // If failed, it won't do attributes interpolation.
                if (zp < depth_buf[get_index(i, j)])
                {
                    ...
                    // Don't forget normalize the normal vector
                    ...
                    // This is shading point coord. Not view dir.
